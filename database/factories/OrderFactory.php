...
class OrderFactory extends Factory
{
    ...
    public function definition()
    {
        ...
        if ($payment === 'carte') {
            ...
            if ($state_id > 6) {
                $purchaseOrder = mt_rand(0, 6);
            }
        } else if ($payment === 'virement') {
            $state_id = $this->faker->unique()->numberBetween(10000, 90000);
        ...
        return [
            'reference' => strtoupper(mt_rand(0,8)),
            'shipping' => $pick ? 0 : mt_rand(500, 1500) / 100,
            'payment' => $payment,
            'state_id' => $state_id,
            'user_id' =>$this->faker->unique()->numberBetween(10000, 90000),
